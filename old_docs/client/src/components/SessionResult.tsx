...
export default function SessionResult({
  ...
  const getStatusColor = (result: typeof results[0]) => {
    if (!result.isCorrect) return "text-black dark:text-white"; // ✕ = 黒
    if (result.confidenceLevel === "perfect") return "text-green-500"; // ◎ = 緑
    if (result.confidenceLevel === "uncertain") return "text-orange-500"; // ○ = オレンジ
    return "text-red-500"; // △ = 赤
  };

  // スコアを有効数字3桁で表示（1-2桁の場合は小数点以下も表示）
